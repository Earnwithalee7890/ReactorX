...
async function main() {
    ...
    const usdc = await MockToken.deploy("Reactor USDC", "USDC", 6);
    await usdc.waitForDeployment();
    const usdcAddress = await usdc.getAddress();
    console.log("✅ USDC deployed to:", usdcAddress);

    const usdt = await MockToken.deploy("Reactor USDT", "USDT", 6);
    ...
    await usdc.mint(dexAddress, ethers.parseUnits("500000", 6)); // 500k USDC
    await usdt.mint(dexAddress, ethers.parseUnits("500000", 6)); // 500k USDT
    await weth.mint(dexAddress, ethers.parseUnits("100", 18));   // 100 WETH
